...
async function handleNicknameLogin() {
    ...
    // Admin hardcoded fallback — intercept exinia before nicknames lookup
    if (nickname.toLowerCase() === 'exinia') {
...
async function handleAdminLogin(nickname, password) {
    ...
    try {
        await auth.signInWithEmailAndPassword('exinia@fraustrohbach.de', password);
    } catch (e) {
        ...
        if (e.code === 'auth/invalid-credential') {
            ...
            try {
                await auth.createUserWithEmailAndPassword('exinia@fraustrohbach.de', password);
